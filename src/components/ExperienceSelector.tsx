  ...
  return (
    <section className="section-padding bg-coastal-50 border-b border-coastal-100">
      <div className="container-wide">
        <p className="section-eyebrow">Start your Kotor cruise plan</p>
        <h2 className="section-title mt-2">How would you like to experience Kotor?</h2>
        <p className="section-subtitle">
          Choose the path that fits your port day, interests and travel style — each card leads to the most relevant planning guides and excursions.
        </p>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {experienceCards.map((card) => {
            const image = subjectImages[card.imageKey] ?? subjectImages["old-town"];
            return (
              <Link
                key={card.slug}
                href={card.href}
